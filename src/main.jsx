...
function ProgrammeCard({ item }) { return <Reveal className="programme-card"><a className="programme-image-link" href={contactLink(item.title)} aria-label={`Enquire about ${item.title}`}><Placeholder number={item.id} label={item.title} /><ArrowUpRight className="card-arrow" size={24} /></a><div className="programme-copy"><span className="micro-label">{item.type} PATHWAY</span><h3><a href={contactLink(item.title)}>{item.title}</a></h3><p>{item.description}</p><a className="text-link" href={contactLink(item.title)}>Explore this pathway <ArrowUpRight size={17} /></a></div></Reveal>; }
