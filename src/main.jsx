...
import { navigation, titles, audiences, blueprint, differences, values, services, programmes, subjects, scholarships, scholarshipSupport } from './data';
import { blogArticles as guides } from './blog-content';
...
function ProgrammeCard({ item }) { return <Reveal className="programme-card"><a className="programme-image-link" href={contactLink(item.title)} aria-label={`Enquire about ${item.title}`}><Placeholder number={item.imageNumber} label={item.title} /><ArrowUpRight className="card-arrow" size={24} /></a><div className="programme-copy"><span className="micro-label">{item.type.toUpperCase()} PATHWAY</span><h3><a href={contactLink(item.title)}>{item.title}</a></h3><p>{item.description}</p><a className="text-link" href={contactLink(item.title)}>Explore this pathway <ArrowUpRight size={17} /></a></div></Reveal>; }
...
function BlogCards() {
  return <div className="programme-grid blog-list">{guides.map(g => <Reveal key={g.id} className="blog-card"><a href={`#/blog?article=${g.id}`} aria-label={`Read ${g.title}`}><Placeholder number={g.imageNumber} label={g.category.toLowerCase()} /></a><div className="blog-card-copy"><span className="micro-label">{g.category}</span><span className="blog-reading-time">{g.readTime}</span><h3><a href={`#/blog?article=${g.id}`}>{g.title}</a></h3><p>{g.excerpt}</p><a className="text-link" href={`#/blog?article=${g.id}`}>Read more <ArrowUpRight size={18} /></a></div></Reveal>)}</div>;
}
function Home() {
  return <><Hero />
    ...
    <section className="section blueprint-section"><SectionHead label="THE CRED CAREER BLUEPRINT" title="Your Personal Education and Career Roadmap" description="We assess your education, experience, career goals, schedule and budget before recommending a suitable academic, professional and skills-based pathway." /><div className="blueprint-grid"><ImpactCards /><Reveal><ol className="roadmap-list">{['Understand your current profile', 'Define your career direction', 'Select suitable qualifications', 'Identify important skills gaps', 'Build your progression plan'].map((step,i) => <li key={step}><span>0{i+1}</span><h3>{step}</h3></li>)}</ol><Button href={contactLink('CRED Career Blueprint')}>Build My Career Blueprint</Button></Reveal></div></section>
...
function App() {
 ...
 useEffect(() => {
   ...
   const update = () => {
     ...
     const sections = [...document.querySelectorAll('main section, main > article, footer')];
